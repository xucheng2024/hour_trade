...
try:
    ...
    if log_dir.exists() and log_dir.is_dir():
        ...
        handler = RotatingFileHandler(
            filename=str(log_file), maxBytes=100 * 1024 * 1024, backupCount=3
        )
...
def process_data(data):
    ...
    for each in data:
        ...
        if "filled" not in state or side != "buy":
            exit()
            ...
            def send_ping():
                while True:
                    ...
                    try:
                        ws.send(
                            "ping"
                        )  # Send empty message, equivalent to sending ping message
...
def on_message(ws, msg_string):
    ...
    try:
        ...
        #     pass
            ...
            hourly_time = current_time.replace(
