...
def pre_sell(instId,marketDataAPI):
    ...
    if not result:
        logging.error("Failed to retrieve candlestick data after %d attempts", candle_attempts)
        return None
    ...
    if cur_close < last_low : return -1
    else:
        return 1
...
for initial_attempt in range(initial_attempts):
    try:
        ...
        tradeAPI_dict['master'] = TradeAPI(master_apikey, master_secretkey, master_passphrase, False, flag)
...
def main():
    ...
    while True:
        now = datetime.now()
        if is_checked is not None and  is_checked == now.second//10:continue
        ...
        df = pd.DataFrame(rows, columns=['instId', 'flag', 'ordId', 'create_time', 'orderType', 'state', 'price', 'size', 'sell_time','side'])
        ...
        if len(df) > 0:
            ...
            for index,each in df.iterrows():
                ...
                if instId not in currency_dict:continue

                ordId = each['ordId']
                size = float(each['size'])

                real_size = currency_dict[instId]
                if size>real_size or (real_size>size and real_size<1.5*size):size = real_size

                flag = each['flag']

                is_sell = pre_sell(instId,marketDataAPI)
                if is_sell>0:continue
