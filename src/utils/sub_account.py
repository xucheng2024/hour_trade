...
def transfer_sub(from_acc, to_acc, subAccountAPI):
    ...
    for attempt in range(attempts):
        try:
            balance_details = subAccountAPI.get_account_balance(subAcct=from_acc)[
                "data"
            ][0]["details"]
            break
        except Exception as e:
...
def transfer_sub_all(to_acc, sub_AccountAPI):
    ...
    acc_lists = attempt_with_retries(
        sub_AccountAPI.get_subaccount_list, attempts=3, delay=1
    )
    if not acc_lists:
    ...
    for each_acc in acc_data:
        ...
        balance_details = attempt_with_retries(
            sub_AccountAPI.get_account_balance, attempts=3, delay=1, subAcct=label
        )
