...
class BlacklistManager:
    ...
    def get_blacklisted_cryptos(self) -> Set[str]:
        ...
        try:
            if not self.db_config:
                self.logger.warning(
                    "⚠️ Database credentials not fully configured, skipping blacklist check"
                )
                return set()
            ...
            with psycopg.connect(self.db_config) as conn:
                with conn.cursor(row_factory=dict_row) as cursor:
                    ...
                    self.logger.info(
                        f"📋 Loaded {len(blacklisted)} blacklisted cryptocurrencies: {sorted(blacklisted)}"
                    )
                    return blacklisted
    ...
    def get_blacklist_reason(self, crypto_symbol: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error getting blacklist reason for {crypto_symbol}: {e}"
            )
            return None
