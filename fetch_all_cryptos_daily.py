#!/usr/bin/env python3
"""
Fetch daily data for all cryptocurrencies in the selected list
Similar to BTC data fetching but for all coins
"""
...
def fetch_all_crypto_data():
    ...
    print(f"🚀 Starting data fetch for {len(cryptos)} cryptocurrencies")
    print("=" * 60)
    ...
    for i, crypto in enumerate(cryptos, 1):
        print(f"\n📊 [{i}/{len(cryptos)}] Processing {crypto}...")
        ...
        if os.path.exists(data_file):
            ...
            try:
                data = dm.get_historical_data(crypto, '1D')
                if data is not None:
